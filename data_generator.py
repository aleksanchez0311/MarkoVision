    ...
    def generate_ohlcv(
        ...
        for i in range(1, n_bars):
            ...
            daily_return = np.random.normal(
                params['drift'], 
                volatility * params['vol_mult']
            )
            
            new_close = close_prices[-1] * (1 + daily_return)
            
            # Generar high/low realista
            hl_range = new_close * volatility * params['vol_mult'] * np.random.uniform(0.5, 1.5)
...
class FeatureEngineer:
    ...
    def _calculate_momentum_slope(self, df: pd.DataFrame, period: int = 20) -> pd.Series:
        """
...
if __name__ == "__main__":
    print("=" * 60)
    print("MARKOVLENS PRO - Generador de Datos de Mercado")
    print("=" * 60)
